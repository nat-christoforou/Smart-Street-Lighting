/* This is uploaded to the nodes that are not the central one */
...
{
    int recvd;
    int len = packetbuf_copyto(&recvd);
    /*printf("Received %d bytes from %d.%d with payload: %d\n",len,
	     sender->u8[0], sender->u8[1], recvd);*/
    
    process_post(&multihop_process, PROCESS_EVENT_MSG , &(recvd) );
	...
	const linkaddr_t *prevhop, uint8_t hops)
{
    ...
    /*printf("Received %d bytes from %d.%d with payload: %d\n",len,
	     originator->u8[0], originator->u8[1], recvd);
    
    printf("dest->u8[0] : %d\n", dest->u8[0]);
    printf("linkaddr_node_addr.u8[0]: %d\n", linkaddr_node_addr.u8[0]);
    printf("dest->u8[1] : %d\n", dest->u8[1]);
    printf("linkaddr_node_addr.u8[1]: %d\n", linkaddr_node_addr.u8[1]);*/
    
    if (dest->u8[0] == linkaddr_node_addr.u8[0] && dest->u8[1] == linkaddr_node_addr.u8[1]){
...
PROCESS_THREAD(multihop_process, ev, data)
{
  ...
  /* Loop forever */
   while(1) {
	...
	light = *(int*)data; // Brightness value we receive from central node
    motion = adc_zoul.value(ZOUL_SENSORS_ADC3); // Measure motion
    //printf("Light: %d\n", light);
    //printf("Motion: %d\n", motion);
